...
export default function DirectorHome() {
  ...
  const availClashes = useMemo(() => {
    ...
    for (const [k, list] of byPersonDay) {
      ...
      const av = availability.find(a => a.userId === userId && a.date === date);
      if (!av) continue;                       // nothing submitted — never flagged
  ...
  const tasks = [
    ...
    availClashes.length > 0 && {
      ...
      sub: `${availClashes[0].name} · ${fmtDay(availClashes[0].date, { weekday: 'short', month: 'short', day: 'numeric' })} — ${describeConflict(availClashes[0]) || 'outside their hours'}`,
